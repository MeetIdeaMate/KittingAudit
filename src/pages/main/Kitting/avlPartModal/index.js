        ...
        <Skeleton active={isLoading} loading={isLoading}>
            ...
            <div style={{ width: "100%", display: "flex" }}>
                <div style={{ width: "50%" }}>
                    ...
                    <hr></hr>
                    {
                        selectedPartDetails?.caseInfo?.[0]?.barcodes?.map((part, indx, arr) => {
                            return <p key={indx} style={{ borderBottom: indx !== arr.length - 1 ? "1px solid #ccc" : "none" }}>{part?.part}</p>
                        })
                    }
                </div>
                ...
                <div style={{ width: "49%" }}>
                    ...
                    {
                        selectedPartDetails?.caseInfo?.[0]?.barcodes?.map((part, indx, arr) => {
                            return <p key={indx} style={{ paddingLeft: "5px ", borderBottom: indx !== arr.length - 1 ? "1px solid #ccc" : "none" }}>{part?.part}</p>
                        })
                    }
                </div>
            </div>
