...
import CableMainBarcode from "./template/cableMainBarcodel";
...
const PrintMasterPdf = React.forwardRef((props, ref) => (
    <div ref={ref} className="rotate-print">
        {
            props?.packingLabelSize === "210*297" ? <CableMainBarcode stickers={props?.stickers} vendorName={props?.vendorName} /> : <MainBarcode stickers={props.stickers} />
        }
    </div>
));
...
export const Kitting = () => {
    ...
    const [packingLabelSize, setPackingLabelSize] = useState("");
    ...
    useQuery(["FETCH_PARENT_LABEL_SIZE", ""], () => api.get(`${CONFIG}/PackingLabelSize`), {
        enabled: true,
        refetchOnWindowFocus: false,
        onSuccess: (configResponse) => {
            setPackingLabelSize(configResponse?.configuration?.[0]);
        }
    });
    ...
    @page {
      ...
      margin: ${packingLabelSize === "210*297"?"10px 30px":"0"} ;;
    }
      html,body{
    ...
    return (
        <React.Fragment>
            ...
            <div style={{ display: "none", width: "100%" }}>
                <PrintMasterPdf ref={masterStickerRef} stickers={mainPartPdfDetails} vendorName={vendorName} packingLabelSize={packingLabelSize} />
                <PrintSticker
                    ref={stickerRef}
